...
describe 'Parses MIXAL expressions' do
  before(:each) do
    ...
    @mock_evaluator.stub(:resolve_constant)
  ...
  it 'should evaluate a symbol' do
    @mock_evaluator.should_receive(:resolve_constant).with('CIRO').and_return(1)
    @parser.evaluate('CIRO').should eq 1
  end
  it 'should evaluate an asterisk' do
    @mock_evaluator.should_receive(:resolve_constant).with('*').and_return(1)
  ...
  it 'should evaluate an expression with *' do
    @mock_evaluator.should_receive(:resolve_constant).with('CIRO').and_return(10)
  ...
  it 'should evaluate a triple asterisk expression' do
    @mock_evaluator.should_receive(:resolve_constant).with('*').and_return(2)
    @mock_evaluator.should_receive(:resolve_constant).with('*').and_return(2)
    @parser.evaluate('***').should eq 4
  end
  it 'should evaluate an asterisk expression' do
    @mock_evaluator.should_receive(:resolve_constant).with('*').and_return(7)
